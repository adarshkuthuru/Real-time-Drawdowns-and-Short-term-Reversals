...
snp=read.csv(file.choose(),header = T) #sp500 file from 
#E:\Drive\Realtime_DD_iteration1\
...
for(i in 1:nrow(Wed)){ 
  ...
  #Updated drawdowns (after the drawdown date when there is a change in peak)
  ...
  if(t==j){
    ...
    for(k in (t+1):(as.numeric(Wed[i+1,3]))){
      ...
      Peak_to_peak_DD[k,2]=as.numeric(Peak_to_peak_DD[k-1,2])+log(as.numeric(snp[k,2])/as.numeric(snp[k-1,2])) #drawdown with respect to peak
    }
  }
  
  #Updated drawdowns (before the drawdown date when there is a change in peak)
  if(i>1){
    for(l in (as.numeric(Wed[i-1,3])+1):(t-1)){
      if(is.na(Peak_to_peak_DD[l,2])==0){
      } else{
        Peak_to_peak_DD[l,1]=as.character(snp[l,1])
        Peak_to_peak_DD[l,2]=as.numeric(Peak_to_peak_DD[l-1,2])+log(as.numeric(snp[l,2])/as.numeric(snp[l-1,2])) #drawdown with respect to peak
      }
    }
  }
}
...
#filling row numbers of candidate troughs
for(i in 1:nrow(Realtime_DD)){
  if(is.na(Realtime_DD[i,2])==0 & is.na(Realtime_DD[i,4])==1){
    Realtime_DD[i,4]=grep(Realtime_DD[i,2],Peak_to_peak_DD[,1])[1]
  }
}


################################
#Estimation of real-time drawdowns
Peak_prc=snp[1,2]
Trough_prc=snp[1,2]
...
#if trough is repeated twice, change drawdowns: Final drawdowns (Real-time)
#Peak_prc=snp[1,2]
a=0
b=0
for(i in 1:nrow(Realtime_DD)){ 
  for(z in 1:(nrow(Wed)-1)){
    for(j in (as.numeric(Wed[z+1,3])+1):(as.numeric(Wed[z+2,3]))){
      ...
      if(a==b & a!=0){  #repeated twice
        Realtime_DD[a,3]=0
        for(k in (a+1):nrow(Realtime_DD)){
          if(Realtime_DD[k,3]==0){ #if peak then break
            break
          } else{
...
Rt_dd=cbind(Peak_to_peak_DD,Realtime_DD[,1:3])
colnames(Rt_dd)=c("Date","Peak-to-peak Drawdowns","Prices","Candidate trough","Real-time Drawdowns")
write.csv(Rt_dd,"Rt_dd_2019-05-11.csv")

##Make corrections in real-time drawdowns 

#Case1: two different troughs between consecutive peaks
#Case2: need not have to change anything if there is a peak between two distinct troughs
#Case3: need not have to change anything if there is one trough between two peaks


# DD_new=matrix(NA,nrow=nrow(Realtime_DD),ncol=1)
# 
# count=1
# t1=0
# t=0
# max=0
# t2=0
# t3=2 #starts from row-2
# for(i in 1:(nrow(Rt_dd)-1)){ #
#   t1=i
#   if(Rt_dd[i,3]>max){
#     max=Rt_dd[i,3]
#     t2=i
...
#     t=Rt_dd[i,4]
#     for(j in (i+1):nrow(Rt_dd)){
#       if(Rt_dd[j,3]>max){
#         max=Rt_dd[j,3]
#         t2=j
#       }
#       if(is.na(Rt_dd[j,4])==0 & Rt_dd[j,4]==t){
#         count=count+1
#         for(k in t3:nrow(Rt_dd)){if(Rt_dd[j,4]==Rt_dd[k,1]){
#           t3=k #row number of trough from price column
#           break}
#           } 
#           i=j
#       }
#       if(is.na(Rt_dd[j,4])==0 & Rt_dd[j,4]!=t){break}
#     }
#     if(t2<t3){ #if peak came before trough
#       if(count>=2 & Rt_dd[i,5]!=0){ #if candidate trough is repeated and no peak on that day
#         for(z in (i+1):nrow(Rt_dd)){
#           if(Rt_dd[z,5]==0){break} else{DD_new[z,1]=as.numeric(Rt_dd[z-1,5])+snp[z,3]}
#         }
#       }
#     }
#   }
...
# }
# 
# colnames(DD_new)=c("DD_new")
# Rt_dd1=cbind(Rt_dd,snp[,3],DD_new)
...
#     }
#     
#     if(a==b & a!=0){  #trough repeated twice
...
# write.csv(Rt_dd1,file="Realtime_DD_RepeatedThrice.csv")
...
t2=0
for(i in 1:nrow(DD_strategy)){
  if(DD_strategy[i,11]<=0){new=new+DD_strategy[i,11]}
  if(DD_strategy[i,11]>0){
    t=new
    if(t<t2){t2=t}
    new=0
  }
